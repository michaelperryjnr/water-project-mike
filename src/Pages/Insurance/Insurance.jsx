...
import InsuranceDropdown1 from "../../Components/Dropdown/InsuranceDropdown1";

const Insurance = () => {
  const [entity, setEntity] = useState([]); // Initialize state for vehicle data
  ...
  useEffect(() => {
    // Fetch vehicle data from API when the component mounts
    axios
      .get(endPointUrl) // Replace with your actual API endpoint
      .then((response) => {
  ...
  const filteredData = useMemo(() => {
    return Array.isArray(entity) ? entity.filter((row) => {
      ...
      : []; // fallback to empty array if Insurance is not an array
  }, [search, entity]);
  ...
  const columns = useMemo(() => [
      {
        accessorKey: "policyNumber", // Custom accessor for full name
        header: () => <span>Policy Number</span>,
        cell: (info) => {
          return info.row.original.policyNumber || "N/A";
        },
      },
      {
        accessorKey: "provider", // Access brand and model name
        header: () => <span>Insurance Provider</span>,
        cell: (info) => {
          return info.row.original.provider || "N/A";
        },
      },
      {
        accessorKey: "insuranceDescription", // Access status
        header: () => <span>Insurance Type</span>,
        cell: (info) => {
          return info.row.original.insuranceType || "N/A";
        },
      },
      {
        accessorKey: "actions",
        header: () => <span>Actions</span>,
        cell: (info) => <InsuranceDropdown1 row={info.row.original} />,
      },
    ],
    []);
  ...
  const handleExport = (format) => {
    switch (format) {
      case "csv":
        exportToCSV(filteredData, "Insurance-data");
        break;
      case "xlsx":
        exportToXLSX(filteredData, "Insurance-data");
        break;
      case "docx":
        exportToDOCX(filteredData, "Insurance-data");
        break;
      default:
        break;
    }
