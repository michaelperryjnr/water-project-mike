...
import PositionDropdown from '../../Components/Dropdown/PositionDropdown';
    ...
    const columns = useMemo(() => [
      {
        ...
        cell: info => {
          const {positionTitle} = info.row.original;
          return `${positionTitle}`;
        },
      },
      {
        accessorKey: 'positionDescription',  // Access position title
        header: () => <span>Position Description</span>,
        cell: info => {
          return info.row.original.positionDescription || 'N/A';
        },
      },
      {
        accessorKey: 'positionBaseSalary',  // Access department name
        header: () => <span>Base Salary</span>,
        cell: info => {
          return info.row.original.positionBaseSalary || 'N/A';
        },
      },
      {
        accessorKey: 'actions',
        ...
        cell: info => (
          <PositionDropdown row={info.row.original} />
        ),
      },
    ], []);
  ...
  return (
    <DefaultLayout>
        ...
        <div className='flex justify-end mb-4'>
          ...
          <Link to={'/test#position-form'} className='text-center py-3 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-taureanOrange text-white hover:bg-taureanLightBlue disabled:opacity-50 disabled:pointer-events-none'>
            Add Position
            <BsPlus size={24} />
          </Link>
        </div>
