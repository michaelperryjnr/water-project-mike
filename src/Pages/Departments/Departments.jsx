...
const Departments = () => {
    ...
    const columns = useMemo(() => [
      {
        ...
        cell: info => {
          const {departmentName} = info.row.original;
          return `${departmentName}`;
        },
      },
      {
        accessorKey: 'departmentDescription',  // Access position title
        header: () => <span>Department Description</span>,
        cell: info => {
          return info.row.original.departmentDescription || 'N/A';
        },
      },
      {
        accessorKey: 'departmentHead',  // Access department name
        header: () => <span>Department Head</span>,
        cell: info => {
          return info.row.original.departmentHead || 'N/A';
        },
      },
      {
        accessorKey: 'actions',
        ...
        cell: info => (
          <DepartmentDropdown row={info.row.original} />
        ),
      },
    ], []);
    ...
    const handleExport = (format) => {
      switch (format) {
        case 'csv':
          exportToCSV(filteredData, 'Position-data');
          break;
        case 'xlsx':
          exportToXLSX(filteredData, 'Position-data');
          break;
        case 'docx':
          exportToDOCX(filteredData, 'Position-data');
          break;
        default:
          break;
      }
  ...
  return (
    <DefaultLayout>
        ...
        <div className='flex justify-end mb-4'>
          ...
          <Link to={'/test#department-form'} className='text-center py-3 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-taureanOrange text-white hover:bg-taureanLightBlue disabled:opacity-50 disabled:pointer-events-none'>
            Add Department
            <BsPlus size={24} />
          </Link>
        </div>
