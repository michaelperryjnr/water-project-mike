...
import VehicleDropdown from "../../Components/Dropdown/VehicleDropdown";
...
const Vehicles = () => {
  ...
  const filteredData = useMemo(() => {
    return Array.isArray(entity)
      ...
      : []; // fallback to empty array if vehicles is not an array
  }, [search, entity]);
    ...
    () => [
      {
        ...
        cell: (info) => {
          const { registrationNumber } = info.row.original;
          return `${registrationNumber || "N/A"}`;
        },
      },
      {
        accessorKey: "brand and model", // Access brand and model name
        header: () => <span>Model & Manufacturer</span>,
        cell: (info) => {
          const { brand, model } = info.row.original;
          return `${brand?.name || "N/A"} ${model || "N/A"}`;
        },
      },
      {
        accessorKey: "status", // Access status
        header: () => <span>Vehicle Status</span>,
        cell: (info) => {
      ...
      },
      {
        ...
        cell: (info) => <VehicleDropdown row={info.row.original} />,
      },
    ],
    []
  );
  ...
  const handleExport = (format) => {
    switch (format) {
      case "csv":
        exportToCSV(filteredData, "Vehicle-data");
        break;
      case "xlsx":
        exportToXLSX(filteredData, "Vehicle-data");
        break;
      case "docx":
        exportToDOCX(filteredData, "Vehicle-data");
        break;
      default:
        break;
    }
