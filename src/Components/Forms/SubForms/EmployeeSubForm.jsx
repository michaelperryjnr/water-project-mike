...
function EmployeeSubForm() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      console.log("Raw value before processing:", value); // Log the initial value
      ...
      const deduplicatedValue = Object.fromEntries(
        Object.entries(value).map(([key, val]) => [
          key,
          Array.isArray(val) ? val[0] : val, // Take the first value if it's an array
        ])
      );

      console.log("Deduplicated value:", deduplicatedValue); // Log after deduplication

      // Convert and clean the data
      const convertedData = {
        ...deduplicatedValue,
        isFullTime: deduplicatedValue.isFullTime === true || deduplicatedValue.isFullTime === "true",
        confirmed: deduplicatedValue.confirmed === true || deduplicatedValue.confirmed === "true",
        probationStarted: deduplicatedValue.probationStarted === true || deduplicatedValue.probationStarted === "true",
        terminated: deduplicatedValue.terminated === true || deduplicatedValue.terminated === "true",
        salary: parseFloat(deduplicatedValue.salary),
        probationPeriod: deduplicatedValue.probationPeriod ? parseInt(deduplicatedValue.probationPeriod) : undefined,
      };

      console.log("Converted data:", convertedData); // Log after conversion
      ...
      for (const [key, value] of Object.entries(dataWithoutStaffNumber)) {
        if (value !== undefined && value !== "") {
          formData.append(key, value);
          console.log(`Appended ${key}: ${value}`); // Log each append
        }
      }
      ...
      if (extFiles.length > 0) {
        ...
        console.log("Appended picture:", extFiles[0].file.name); // Log file append
      }
  ...
  const updateFiles = (incomingFiles) => {
    console.log("Updated files:", incomingFiles); // Debug Dropzone updates
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => {
                      console.log("Title change:", e.target.value); // Debug
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      console.log("FirstName change:", e.target.value); // Debug
                      field.handleChange(e.target.value);
                    }}
                    className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-taureanOrange focus:ring-taureanOrange disabled:opacity-50 disabled:pointer-events-none"
                    placeholder="John"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      console.log("MiddleName change:", e.target.value); // Debug
                      field.handleChange(e.target.value);
                    }}
                    className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-taureanOrange focus:ring-taureanOrange disabled:opacity-50 disabled:pointer-events-none"
                    placeholder="Doe"
